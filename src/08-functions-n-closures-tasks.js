...
// console.log(getComposition(Math.sin, Math.asin)(x));
...
function getPolynom(...args) {
  return (x) => args.reduce((acc, item, index) => acc + item * x ** (args.length - index - 1), 0);
}
...
function getIdGeneratorFunction(startFrom) {
  const arr = [];
  const func = (newStart = startFrom) => {
    if (!arr.includes(newStart)) arr.push(newStart);
    else return func(newStart + 1);
    return newStart;
  };
  return func;
}
